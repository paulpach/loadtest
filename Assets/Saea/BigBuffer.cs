...
namespace Mirror.Saea
{
    public class BigBuffer
    {
        ...
        // average it's 1 send + 1 recv chunk for 1k connections = 2k chunks.
        // -> no Unity game will run with >10k connections anyway.
        // -> chunk size seems to affect performance. 1k*2k works well.
        const int ChunkAmount = 1024;
        readonly byte[] buffer = new byte[ChunkSize * ChunkAmount];

        // free chunks - by offset. threadsafe.
        // -> contains all the indices initially. e.g. 0, 1024, 2048, ...
        readonly ConcurrentQueue<int> freeOffsets = new ConcurrentQueue<int>(Enumerable.Range(0, ChunkAmount).Select(idx => idx * ChunkSize));
        ...
        public bool Assign(SocketAsyncEventArgs args)
        {
            ...
            if (freeOffsets.TryDequeue(out int offset))
            {
                // assign to args
            ...
            Debug.LogError("BigBuffer.Assign: out of free indices. Should've allocated more than " + ChunkAmount + ".");
            return false;
        }
        ...
        public bool Assign(SocketAsyncEventArgs args, byte[] header, byte[] content)
        {
            ...
            if (freeOffsets.TryDequeue(out int offset))
            {
                ...
                Debug.LogError("BigBuffer.Assign: can't copy " + totalLength + " bytes into chunk of max size " + ChunkSize + " for header=" + BitConverter.ToString(header) + " content=" + BitConverter.ToString(content));
                return false;
            }
        ...
        public void Free(SocketAsyncEventArgs args)
        {
            ...
            if (freeOffsets.Count % 1000 == 0)
                Debug.Log("BigBuffer free indices: " + freeOffsets.Count);
        }
    }
}